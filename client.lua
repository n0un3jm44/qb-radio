...
local function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        esx:showAdvancedNotification(Config.messages['joined_to_radio'] ..channel.. ' MHz', 'success')
    else
        esx:showAdvancedNotification(Config.messages['joined_to_radio'] ..channel.. '.00 MHz', 'success')
    end
end
...
local function leaveradio()
    ...
    esx:showAdvancedNotification(Config.messages['you_leave'] , 'error')
end
        ...
        if rchannel <= Config.MaxFrequency and rchannel ~= 0 then
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    if Config.RestrictedChannels[rchannel][xPlayer.getJob] then
                        connecttoradio(rchannel)
                    else
                        esx:showAdvancedNotification(Config.messages['restricted_channel_error'], 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                esx:showAdvancedNotification(Config.messages['you_on_radio'] , 'error')
            end
        else
            esx:showAdvancedNotification(Config.messages['invalid_radio'] , 'error')
        end
    else
        esx:showAdvancedNotification(Config.messages['invalid_radio'] , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(data, cb)
    if RadioChannel == 0 then
        esx:showAdvancedNotification(Config.messages['not_on_radio'], 'error')
    else
        leaveradio()
    end
end)
...
RegisterNUICallback("volumeUp", function()
	if RadioVolume <= 95 then
		...
		esx:showAdvancedNotification(Config.messages["volume_radio"] .. RadioVolume, "success")
		exports["pma-voice"]:setRadioVolume(RadioVolume)
	else
		esx:showAdvancedNotification(Config.messages["decrease_radio_volume"], "error")
	end
end)
...
RegisterNUICallback("volumeDown", function()
	if RadioVolume >= 10 then
		...
		esx:showAdvancedNotification(Config.messages["increase_radio_volume"], "error")
	end
end)
...
RegisterNUICallback("increaseradiochannel", function(data, cb)
    ...
    esx:showAdvancedNotification(Config.messages["increase_decrease_radio_channel"] .. newChannel, "success")
end)
